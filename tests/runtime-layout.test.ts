...
const roots: string[] = [];
afterEach(() => {
  delete process.env.COWEB_HOME;
  for (const root of roots.splice(0)) rmSync(root, { recursive: true, force: true });
});

test("managed runtime commands reject every ephemeral path component", () => {
  expect(() => assertDurableRuntimeCommand(["/private/tmp/coweb"])).toThrow("ephemeral path");
...
test("installed Bun discovery ignores a temporary self-extract executable", () => {
  const root = join(tmpdir(), `coweb-bun-discovery-${process.pid}-${Date.now()}`);
...
test("Windows uses a stable native named pipe for the outer Codex tool broker", () => {
  const first = defaultBrokerEndpoint("C:\\Users\\alice\\.coweb", "win32");
  const second = defaultBrokerEndpoint("C:\\Users\\alice\\.coweb", "win32");
  expect(first).toBe(second);
  expect(isWindowsPipeEndpoint(first)).toBe(true);
  expect(resolveBrokerEndpoint(first)).toBe(first);
  expect(defaultBrokerEndpoint("/home/alice/.coweb", "linux")).toEndWith(join("runtime", "turn-broker.sock"));
});
...
test("setup repairs a legacy automatic connector name that collides with Zero Risk", () => {
  const root = join(tmpdir(), `coweb-connector-collision-${process.pid}-${Date.now()}`);
  roots.push(root);
  process.env.COWEB_HOME = root;
  mkdirSync(root, { recursive: true });
...
test("setup explicitly migrates v1 pro-only config to v3 managed browser-only", () => {
  const root = join(tmpdir(), `coweb-config-migration-${process.pid}-${Date.now()}`);
  roots.push(root);
  process.env.COWEB_HOME = root;
  mkdirSync(root, { recursive: true });
...
test("existing v3 configurations deterministically retain automatic browser interaction", () => {
  const root = join(tmpdir(), `coweb-v3-interaction-migration-${process.pid}-${Date.now()}`);
  roots.push(root);
  process.env.COWEB_HOME = root;
  mkdirSync(root, { recursive: true });
...
test("Zero Risk fails closed without the Launcher browser host", () => {
  const root = join(tmpdir(), `coweb-manual-host-${process.pid}-${Date.now()}`);
  roots.push(root);
  process.env.COWEB_HOME = root;
  mkdirSync(root, { recursive: true });
...
test("legacy temp-path wrapper and vendor are removed only after runtime ownership changes", () => {
  const root = join(tmpdir(), `coweb-legacy-runtime-${process.pid}-${Date.now()}`);
  roots.push(root);
  process.env.COWEB_HOME = root;
...
test("launcher browser ownership is explicit in provider configuration", () => {
  ...
  config.browserHostDescriptorPath = "/Users/example/.coweb/runtime/launcher-browser.json";
