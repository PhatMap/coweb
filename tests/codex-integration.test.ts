...
function fixture(): { root: string; codexHome: string; appHome: string } {
  const root = join(tmpdir(), `coweb-integration-${process.pid}-${Date.now()}-${Math.random()}`);
  const codexHome = join(root, "codex");
  const appHome = join(root, "app");
  mkdirSync(codexHome, { recursive: true });
  roots.push(root);
  process.env.CODEX_HOME = codexHome;
  process.env.COWEB_HOME = appHome;
...
describe("reversible native Codex route integration", () => {
  ...
  test("keeps the built-in openai provider without changing native feature defaults", () => {
    ...
    expect(installed).not.toContain("[model_providers.coweb]");
  ...
  test("Compatibility V1 owns both subagent feature flags and restores the exact user lines", () => {
    ...
    expect(installed).toContain("multi_agent = true # Managed by coweb");
    expect(installed).toContain("multi_agent_v2 = false # Managed by coweb");
    ...
    expect(readFileSync(configPath, "utf8")).toContain("multi_agent_v2 = false # Managed by coweb");
  ...
  test("Compatibility V1 preserves a structured multi_agent_v2 table", () => {
    ...
    const installed = readFileSync(configPath, "utf8");
    expect(installed).toContain("enabled = false # Managed by coweb");
