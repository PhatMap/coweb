...
const ZERO_RISK_MCP_INSTRUCTIONS = [
  "For each pasted CoWeb request, begin with codex_turn_start using the request_id in its request block.",
  ...
  if (contract === "safe") {
    server.registerTool(
      "codex_turn_start",
      {
        ...
        description: "Connect the request_id included in the pasted CoWeb request so its Codex tools can be used.",
        inputSchema: {
          request_id: turnTokenSchema,
        },
        outputSchema: {
