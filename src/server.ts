...
const reportHttpStreamFailure: HttpStreamFailureReporter = evidence => {
  console.warn(`[coweb] http_stream_failed ${JSON.stringify(evidence)}`);
};
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === "GET" && url.pathname === "/v1/models") {
        if (draining) {
          return formatErrorResponse(
            503,
            "server_error",
            "coweb is draining for a requested service operation",
          );
        }
      ...
      if (req.method === "POST" && url.pathname === "/v1/responses") {
        if (draining) return formatErrorResponse(503, "server_error", "coweb is draining for a requested service operation");
    ...
    shutdownPromise = (async () => {
      ...
      if (failures.length > 0) {
        ...
        for (const failure of failures) {
          console.error(`[coweb] shutdown cleanup failed: ${failure instanceof Error ? failure.message : String(failure)}`);
        }
      }
      await server.stop(true);
    })().catch(error => {
      process.exitCode = 1;
      console.error(`[coweb] server shutdown failed: ${error instanceof Error ? error.message : String(error)}`);
    });
  }
