...
const HELP = `coweb ${VERSION}

Focused ChatGPT web-backed models for the native Codex harness.

Usage:
  coweb setup --browser-only [options]
  coweb setup --full --tunnel-id ID --runtime-key-file PATH [options]
  coweb login
  coweb doctor [--json]
  coweb route <status|connect|disconnect>
  coweb subagents <status|compatibility-v1|native>
  coweb browser check
  coweb dev launcher
  coweb dev status [--json]
  coweb dev setup <--browser-only|--full> [options]
  coweb dev chat NAME [--model MODEL] [MESSAGE]
  coweb dev list
  coweb serve
  coweb mcp [--broker-socket PATH]
  coweb service <status|install|start|restart|stop|cancel-turns>
  coweb tunnel <status|start|restart|stop|key-import>
  coweb open <tunnels|runtime-keys|connectors>
  coweb uninstall --yes
...
function authorizeLauncherControl(operation: string): void {
  const descriptorPath = process.env.COWEB_BROWSER_HOST_DESCRIPTOR?.trim();
...
async function loginCommand(args: string[]): Promise<void> {
  ...
  if (!launcherControl) {
    ...
    if (config.browserHost === "launcher") {
      throw new Error("ChatGPT login is owned by the launcher; open CoWeb and use its Sign in step");
    }
...
async function uninstallCommand(args: string[]): Promise<void> {
  ...
  if (config?.browserHost === "launcher" && !launcherControl) {
    throw new Error(
      "Launcher-owned integration must be removed from CoWeb Settings so the active runtime can be drained safely.",
    );
  }
...
async function main(): Promise<void> {
  ...
  if (home) process.env.COWEB_HOME = home;
  ...
  if (command === "dev" && home) {
    throw new Error("--home does not apply to DEV mode; use COWEB_DEV_HOME for an explicit isolated DEV profile");
  }
  if (command === "help") stdout.write(HELP);
    ...
    stdout.write(`coweb ${VERSION} listening on http://${config.host}:${server.port}/v1 (${config.mode})\n`);
...
main().catch(error => {
  process.stderr.write(`coweb: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 1;
});
