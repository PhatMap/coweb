...
const DEV_HELP = `CoWeb DEV chat

Usage:
  coweb dev launcher
  coweb dev status [--json]
  coweb dev setup --browser-only [--automatic-browser-interaction]
  coweb dev setup --full --tunnel-id ID --runtime-key-file PATH [--automatic-browser-interaction|--zero-risk-browser-interaction]
...
function printHeader(
  ...
  stdout.write(`${bold("CoWeb DEV")} · ${created ? "created" : "continued"} chat ${cyan(state.name)}\n`);
