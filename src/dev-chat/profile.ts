...
const WINDOWS_LAUNCHER_GUID = "c2391cc7-d604-44c2-91fc-2d8af7e973a3";
...
export function resolveDevProfilePaths({
  ...
  const home = resolve(expandUserPath(
    environment.COWEB_DEV_HOME?.trim() || join(homeDirectory, ".coweb-dev"),
  ));
  const productionHome = resolve(expandUserPath(
    environment.COWEB_HOME?.trim() || join(homeDirectory, ".coweb"),
  ));
...
export function activateDevProfileEnvironment(paths = resolveDevProfilePaths()): DevProfilePaths {
  process.env.COWEB_DEV_HOME = paths.home;
  process.env.COWEB_HOME = paths.home;
...
export function installedLauncherCandidates({
  ...
  const override = environment.COWEB_LAUNCHER_EXECUTABLE?.trim();
  ...
  if (platform === "darwin") {
    candidates.push(
      ...
      posix.join(homeDirectory, "Applications", "CoWeb.app", "Contents", "MacOS", "CoWeb"),
    );
    ...
    if (registeredLocation && win32.isAbsolute(registeredLocation)) {
      candidates.push(win32.join(registeredLocation, "CoWeb.exe"));
    } else {
      const localAppData = environment.LOCALAPPDATA?.trim();
      if (localAppData) {
        candidates.push(win32.join(localAppData, "Programs", "CoWeb", "CoWeb.exe"));
      }
    }
  } else if (platform === "linux") {
    candidates.push(posix.join(homeDirectory, ".local", "bin", "coweb-launcher"));
    for (const entry of (environment.PATH || "").split(":").filter(Boolean)) {
      candidates.push(posix.join(entry, "coweb-launcher"));
    }
  }
...
export function findInstalledLauncherExecutable(options: Parameters<typeof installedLauncherCandidates>[0] = {}): string {
  ...
  throw new Error(
    "Installed CoWeb launcher was not found. Install it first or set COWEB_LAUNCHER_EXECUTABLE to its absolute executable path."
...
export function devLauncherEnvironment(
  ...
  delete childEnvironment.COWEB_HOME;
  delete childEnvironment.CODEX_HOME;
  delete childEnvironment.COWEB_LAUNCHER_DATA_DIR;
  childEnvironment.COWEB_DEV_HOME = paths.home;
