...
export function getConfigDir(): string {
  const configured = process.env.COWEB_HOME?.trim();
  return resolve(expandUserPath(configured || join(homedir(), ".coweb")));
}
...
export function currentRuntimeCommand(): string[] {
  ...
  return runtimeCommandForProcess({
    launcher: process.env.COWEB_CLI_LAUNCHER,
...
export function loadConfig(): AppConfig {
  ...
  if (!existsSync(path)) throw new Error(`Configuration is missing: ${path}. Run coweb setup first.`);
