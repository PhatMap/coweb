...
function prepareSetup(options: SetupOptions): PreparedSetup {
  ...
  if (!launcherOwned && process.platform !== "darwin") {
    throw new Error(
      ...
      + "Use the CoWeb launcher on Windows or Linux.",
    );
  }
...
export async function setup(options: SetupOptions): Promise<SetupResult> {
  ...
  if (beforeService.loaded && !existing) {
    throw new Error("A coweb service is loaded but its configuration is missing; refusing to replace an unverifiable process");
  }
