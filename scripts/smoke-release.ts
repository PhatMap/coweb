...
const root = join(homedir(), `.coweb-release-smoke-${process.pid}-${Date.now()}`);
...
for (const forbidden of [sourceRoot, dirname(sourceBundle), "/private/tmp/coweb-verify", "/tmp/coweb-verify"]) {
...
const env = { ...process.env, COWEB_HOME: appHome, CODEX_HOME: codexHome };
...
try {
  ...
  if (payload.service !== "coweb" || payload.mode !== "browser-only") {
