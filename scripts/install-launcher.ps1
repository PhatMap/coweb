...
$Repository = $env:COWEB_RELEASE_REPOSITORY
if (-not $Repository) { throw "CoWeb release downloads are unconfigured; set COWEB_RELEASE_REPOSITORY explicitly" }
if ($Repository -notmatch '^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$') {
  throw "Invalid GitHub repository: $Repository"
}
$Version = $env:COWEB_RELEASE_VERSION
if (-not $Version) {
...
if (-not $Version) { throw "Could not resolve the latest CoWeb release" }
...
$Asset = "coweb-$Version-win-$Arch.exe"
$BaseUrl = "https://github.com/$Repository/releases/download/v$Version"
$Temp = Join-Path ([System.IO.Path]::GetTempPath()) "coweb-$([guid]::NewGuid().ToString('N'))"
New-Item -ItemType Directory -Path $Temp | Out-Null
try {
  if (Get-Process -Name "CoWeb" -ErrorAction SilentlyContinue) {
    throw "Quit CoWeb before updating it"
  }
  ...
  $InstallRegistry = "HKCU:\Software\c2391cc7-d604-44c2-91fc-2d8af7e973a3"
  ...
  $Executable = Join-Path $InstallLocation "CoWeb.exe"
