...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest CoWeb release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="coweb-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/coweb-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  INSTALL_DIR="${COWEB_APPLICATIONS_DIR:-/Applications}"
  STAGE_DIR="$TEMP_DIR/stage"
  mkdir "$STAGE_DIR"
  ditto -x -k "$TEMP_DIR/$ASSET" "$STAGE_DIR"
  SOURCE_APP="$STAGE_DIR/CoWeb.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/CoWeb" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/CoWeb.app"
  if pgrep -x "CoWeb" >/dev/null 2>&1; then
    echo "Quit CoWeb before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/CoWeb.previous.app"
...
LIB_DIR="${COWEB_LAUNCHER_LIB_DIR:-$HOME/.local/lib/coweb}"
BIN_DIR="${COWEB_LAUNCHER_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/CoWeb.AppImage"
WRAPPER="$BIN_DIR/coweb-launcher"
CORE_HOME="${COWEB_HOME:-$HOME/.coweb}"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "CoWeb\\.AppImage" >/dev/null 2>&1; then
  echo "Quit CoWeb before updating it" >&2
  exit 1
fi
...
RUNNER_QUOTED="$(shell_quote "$RUNNER")"
{
  printf '%s\n' '#!/bin/sh'
  printf '%s\n' 'set -eu'
  printf 'export COWEB_LAUNCHER_EXECUTABLE=%s\n' "$WRAPPER_QUOTED"
  printf 'export COWEB_APPIMAGE=%s\n' "$TARGET_QUOTED"
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/coweb.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/coweb.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=CoWeb
...
StartupWMClass=coweb
EOF
chmod 0644 "$APPLICATIONS_DIR/coweb.desktop"
