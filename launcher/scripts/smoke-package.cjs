...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), "coweb-package-smoke-"));
...
function smokeEnvironment() {
  return {
    ...process.env,
    TMPDIR: scratch,
    COWEB_LAUNCHER_DATA_DIR: path.join(scratch, "launcher-data"),
    COWEB_HOME: coreHome,
...
try {
  ...
  if (process.platform === "darwin") {
    ...
    macAppBundle = path.join(stage, "CoWeb.app");
    executable = path.join(macAppBundle, "Contents", "MacOS", "CoWeb");
    command = executable;
