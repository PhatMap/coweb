...
const USER_AGENT = "coweb-launcher-updater";
...
function releaseAssetName(version, platform = process.platform, arch = process.arch) {
  if (platform === "darwin" && ["arm64", "x64"].includes(arch)) {
    return `coweb-${version}-mac-${arch}.zip`;
  }
  if (platform === "win32" && arch === "x64") {
    return `coweb-${version}-win-x64.exe`;
  }
  if (platform === "linux" && arch === "x64") {
    return `coweb-${version}-linux-x64.AppImage`;
  }
  return null;
}
...
function validateReleaseAssetUrl(raw, version, assetName, repository) {
  if (!repository || !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository)) {
    throw new Error("CoWeb release repository is not configured");
  }
...
function findMacApplication(root) {
  ...
  const executable = path.join(application, "Contents", "MacOS", "CoWeb");
...
function buildJob({ version, platform, executablePath, assetPath, stagingRoot, tempRoot, logPath }) {
  ...
  if (platform === "linux") {
    const target = process.env.COWEB_APPIMAGE?.trim()
    ...
    const wrapper = process.env.COWEB_LAUNCHER_EXECUTABLE?.trim();
...
function defaultDependencies(repository) {
  return {
    fetchRelease: async () => {
      if (!repository) throw new Error("CoWeb release repository is not configured");
      return JSON.parse(await downloadText(`https://api.github.com/repos/${repository}/releases/latest`));
    },
    downloadText,
    downloadFile,
    sha256,
...
function createUpdateController({
  ...
  if (repository !== null && !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository)) {
    throw new Error("Invalid CoWeb release repository");
  }
  const deps = { ...defaultDependencies(repository), ...dependencies };
  const supportedAsset = releaseAssetName(currentVersion, platform, arch);
  let state = repository && packaged && supportedAsset ? { status: "idle" } : { status: "disabled" };
  let checked = false;
  let pending = null;
  let candidate = null;
  ...
  async function beginInstall() {
    ...
    pending = (async () => {
      ...
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "coweb-update-"));
      try {
