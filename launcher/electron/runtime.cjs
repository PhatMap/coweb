...
class RuntimeHost {
  ...
  devSetupEnvironment(environment = process.env) {
    ...
    childEnvironment.COWEB_DEV_HOME = this.coreHome;
  ...
  captureSetupCheckpoint(snapshot) {
    ...
    const paths = new Set([
      ...
      path.join(coreHome, "tunnel", "profiles", "coweb.yaml"),
      path.join(coreHome, "tunnel", "profiles", "coweb-zero-risk.yaml"),
      path.join(coreHome, "tunnel", "profiles", "coweb-dev.yaml"),
      path.join(coreHome, "tunnel", "profiles", "coweb-dev-zero-risk.yaml"),
    ]);
    if (snapshot.owner === "external" && this.platform === "darwin") {
      paths.add(path.join(this.launchAgentsDir, "dev.coweb.daemon.plist"));
      paths.add(path.join(this.launchAgentsDir, "dev.coweb.tunnel.plist"));
    }
    const tunnels = [
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        Object.assign(environment, {
          COWEB_BROWSER_HOST_DESCRIPTOR: this.browserDescriptorPath,
          ...(options.env || {}),
        });
    ...
    try {
      ...
      try {
        const result = await this.run(name, ["uninstall", "--yes", "--launcher-control"], {
          ...
          env: this.launcherControlEnvironment(),
          message: "Restoring the previous Codex route",
          successMessage: "CoWeb integration removed",
