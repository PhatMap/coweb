...
const LINUX_DESKTOP_NAME = "dev.coweb.launcher.desktop";
...
function linuxExecutable(app) {
  const stableLauncher = process.env.COWEB_LAUNCHER_EXECUTABLE?.trim();
  if (stableLauncher && path.isAbsolute(stableLauncher)) return stableLauncher;
  const appImage = process.env.COWEB_APPIMAGE?.trim() || process.env.APPIMAGE?.trim();
...
Version=1.0
Name=CoWeb
Comment=Start the CoWeb launcher in the background
