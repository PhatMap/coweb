...
function updateMac(job) {
  const sourceExecutable = path.join(job.source, "Contents", "MacOS", "CoWeb");
  ...
  requireFile(path.join(next, "Contents", "MacOS", "CoWeb"), "Copied macOS launcher");
...
function updateLinux(job) {
  ...
  fs.writeFileSync(wrapperNext, [
    "#!/bin/sh",
    "set -eu",
    `export COWEB_LAUNCHER_EXECUTABLE=${shellQuote(wrapper)}`,
    `export COWEB_APPIMAGE=${shellQuote(nextTarget)}`,
