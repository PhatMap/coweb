...
process.env.COWEB_HOME = CORE_HOME;
process.env.CODEX_HOME = LAUNCHER_PROFILE.codexHome;
app.setName(LAUNCHER_PROFILE.displayName);
if (process.platform === "win32") {
  app.setAppUserModelId(IS_DEV_PROFILE ? "dev.coweb.launcher.dev" : "dev.coweb.launcher");
}
...
function registerIpc({ logger, stateStore }) {
  ...
  handle("launcher:export-logs", async () => {
    ...
    const result = await dialog.showSaveDialog(mainWindow, {
      ...
      defaultPath: path.join(app.getPath("documents"), `coweb-diagnostics-${date}.jsonl`),
...
async function requestQuit() {
  ...
  try {
    ...
    if (activeOperation) {
      throw new Error(`Wait for ${activeOperation} to finish before quitting CoWeb`);
    }
...
async function start() {
  ...
  if (process.platform === "linux") {
    app.commandLine.appendSwitch("class", IS_DEV_PROFILE ? "coweb-dev" : "coweb");
  }
    ...
    if (runtime.status === "external" || runtime.status === "needs-setup") {
      const detail = runtime.detail || (
        runtime.status === "external"
          ? "Another process owns the configured CoWeb runtime"
...
void start().catch((error) => {
  ...
  try {
    dialog.showErrorBox("CoWeb could not start", message);
  } catch {}
  app.exit(1);
});
