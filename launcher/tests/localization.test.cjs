...
function linkTargets(source) {
  const markdown = [...source.matchAll(/\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g)].map(
    (match) => match[1],
  );
...
test("Japanese launcher runtime messages localize connector verification and doctor success checks", () => {
  ...
  assert.equal(
    localizeRuntimeMessage(copy, "Checking ChatGPT connector", undefined, "ja"),
    "ChatGPT コネクタを確認中",
  );
  assert.equal(
...
for (const language of ["ja", "zh-CN"])
  test(`${language} runtime localization preserves literal connector names and endpoints`, () => {
    const { copyFor, localizeRuntimeMessage } = loadI18nModule();
    ...
    assert.equal(
      ...
      language === "ja"
        ...
        : "Responses 代理在 127.0.0.1:17841 上运行正常",
    );
  });
...
test("Chinese diagnostics cover the same progress and successful checks as Japanese", () => {
  ...
  for (const [id, source, translated] of [
    ...
    [
      "tunnel-binary",
      "Pinned openai/tunnel-client binary is installed",
      "已安装固定版本的 openai/tunnel-client 二进制文件",
    ],
    [
      "tunnel-key",
      "Tunnel runtime key is stored privately",
