...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  assert.equal(manifest.build.appId, "dev.coweb.launcher");
  assert.equal(manifest.build.artifactName, "coweb-${version}-${os}-${arch}.${ext}");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(shellInstaller, /coweb\.desktop/);
  ...
  assert.match(windowsInstaller, /coweb-\$Version-win-\$Arch\.exe/);
  ...
  assert.equal(fullyQualifiedWindowsPath.test("C:\\Users\\tester\\CoWeb"), true);
  assert.equal(fullyQualifiedWindowsPath.test("\\\\server\\share\\CoWeb"), true);
  assert.equal(fullyQualifiedWindowsPath.test("C:CoWeb"), false);
  assert.equal(fullyQualifiedWindowsPath.test("\\CoWeb"), false);
  assert.equal(fullyQualifiedWindowsPath.test("CoWeb"), false);
...
test("Linux AppImage fallback uses one owned extraction and removes it on exit", {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "coweb-appimage-runner-"));
  const runtime = path.join(root, "runtime");
  const appImage = path.join(root, "CoWeb.AppImage");
  ...
  const fallbackRoot = path.join(runtime, `coweb-appimage-${process.getuid?.() ?? 0}`);
