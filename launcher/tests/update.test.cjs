...
test("Linux auto-update fails closed without the stable installer wrapper", () => {
  const previousAppImage = process.env.COWEB_APPIMAGE;
  const previousWrapper = process.env.COWEB_LAUNCHER_EXECUTABLE;
  process.env.COWEB_APPIMAGE = "/opt/codex/CoWeb.AppImage";
  delete process.env.COWEB_LAUNCHER_EXECUTABLE;
  try {
    ...
    if (previousAppImage === undefined) delete process.env.COWEB_APPIMAGE;
    else process.env.COWEB_APPIMAGE = previousAppImage;
    if (previousWrapper === undefined) delete process.env.COWEB_LAUNCHER_EXECUTABLE;
    else process.env.COWEB_LAUNCHER_EXECUTABLE = previousWrapper;
  }
});
...
test("release comparison and platform assets are strict", () => {
  ...
  assert.equal(releaseAssetName("1.2.0", "darwin", "arm64"), "coweb-1.2.0-mac-arm64.zip");
  assert.equal(releaseAssetName("1.2.0", "darwin", "x64"), "coweb-1.2.0-mac-x64.zip");
  assert.equal(releaseAssetName("1.2.0", "win32", "x64"), "coweb-1.2.0-win-x64.exe");
  assert.equal(releaseAssetName("1.2.0", "linux", "x64"), "coweb-1.2.0-linux-x64.AppImage");
...
test("checksums and release URLs bind the exact expected asset", () => {
  ...
  assert.equal(
    validateReleaseAssetUrl(
      "https://github.com/test-owner/coweb/releases/download/v1.2.0/launcher.zip",
      "1.2.0",
      "launcher.zip",
      "test-owner/coweb",
    ),
  ...
  assert.throws(
    () => validateReleaseAssetUrl("https://example.com/launcher.zip", "1.2.0", "launcher.zip", "test-owner/coweb"),
...
test("macOS bundle resolution never guesses outside Contents/MacOS", () => {
  assert.equal(
    macApplicationPath("/Applications/CoWeb.app/Contents/MacOS/CoWeb"),
    "/Applications/CoWeb.app",
  );
  assert.throws(() => macApplicationPath("/tmp/CoWeb"), /Could not resolve/);
});
    ...
    dependencies: {
      fetchRelease: async () => {
        ...
        return {
          ...
          assets: [
            {
              name: "coweb-1.2.0-linux-x64.AppImage",
              browser_download_url: "https://github.com/test-owner/coweb/releases/download/v1.2.0/coweb-1.2.0-linux-x64.AppImage",
            },
            {
              name: "checksums.txt",
              browser_download_url: "https://github.com/test-owner/coweb/releases/download/v1.2.0/checksums.txt",
            },
          ],
        };
      },
...
test("verified update is handed to one detached worker", async () => {
  ...
  const oldAppImage = path.join(root, "versions", "1.1.4", "CoWeb.AppImage");
  const wrapper = path.join(root, "bin", "coweb");
  ...
  const previousAppImage = process.env.COWEB_APPIMAGE;
  const previousWrapper = process.env.COWEB_LAUNCHER_EXECUTABLE;
  process.env.COWEB_APPIMAGE = oldAppImage;
  process.env.COWEB_LAUNCHER_EXECUTABLE = wrapper;
  try {
    const controller = createUpdateController({
    repository: "test-owner/coweb",
      currentVersion: "1.1.4",
      ...
      dependencies: {
        ...
        downloadText: async () => `${hash}  coweb-1.2.0-linux-x64.AppImage\n`,
...
test("detached worker replaces an installed Linux AppImage and removes the old version", {
  ...
  const oldTarget = path.join(versionsRoot, "1.1.4", "CoWeb.AppImage");
  const newTarget = path.join(versionsRoot, "1.2.0", "CoWeb.AppImage");
  const wrapper = path.join(root, "bin", "coweb");
  ...
  try {
    ...
    assert.match(fs.readFileSync(wrapper, "utf8"), /versions\/1\.2\.0\/CoWeb\.AppImage/);
